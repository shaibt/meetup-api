...
class Meetup {
    ...
    getOAuth2AccessToken(oauth_verifier, callback) {
        ...
        // if starting the OAuth dance from the access token stage (request code was delivered externally)
        if (!Meetup.oauth2) {
            let errMessage = (OAuth) ?
                (MeetupAPIkey && MeetupAPIkey.key && MeetupAPIkey.secret) ? null : 'No OAuth Keys' :
                'No OAuth support';
            ...
            req.end(
                (err, res) => {
                    ...
                    if (!err) {
                        ...
                        if (res.header['link']) {
                            let linkComponents = res.header['link'].split(',');
                            response.offsets = {};
                            response.offsets.totalCount = res.header['x-total-count'] || 0;
                            linkComponents.forEach(component => {
                                let linkRel = component.indexOf('rel');
                                if (linkRel >= 0) {
                                    let linkRelValue = component.substr(linkRel+5,4);
                                    let queryData = URLfn.parse(component.substring(component.indexOf('<')+1,component.indexOf('>')),true).query;
                                    let offSet = queryData.offset                                    
                                    if (linkRelValue == 'next') {
                                        response.offsets.next = offSet || 0;
                                    } else if (linkRelValue == 'prev') {
                                        response.offsets.prev = offSet || 0;
                                    }
                                }
                            })
                        }
